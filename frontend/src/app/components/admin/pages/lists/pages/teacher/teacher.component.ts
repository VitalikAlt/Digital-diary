...
export class TeacherListComponent implements OnInit {
  ...
  deleteTeachers() {
    ...
    this.httpService.deleteTeachers(ids).subscribe(
      () => {
        this.ngOnInit();
  ...
  changeDeletedTeachers(teacher) {
    ...
    if (index === -1) this.teachersForDelete.push(teacher);
    else this.teachersForDelete.splice(index, 1);
  }
  ...
  openChangePasswordModal() {
    this.httpService.getTeacherUserData(this.modalUser.id).subscribe(
      result => {
