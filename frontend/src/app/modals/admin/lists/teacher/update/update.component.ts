...
export class UpdateTeacherModal implements OnInit {
  ...
  updateTeacher() {
    ...
    this.httpService.updateTeacherProfile(this.user).subscribe(
      () => {
  ...
  open(user) {
    this.user = user;
    this.photoUrl = `../../../../../../assets/data/photo/teacher/${
      this.user.id
    }.jpg`;
    this.httpService.getTeacherProfile(this.user.id).subscribe(
      result => {
  ...
  upload() {
    ...
    this.httpService
      .uploadUserPhoto(this.filesToUpload, this.user.id, "teacher")
      .then(
        result => {
